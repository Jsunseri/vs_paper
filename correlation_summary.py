...
def plot_iou(df, palette, prefix='', out='', ylabel='', auc={}):
    '''
    ...
    for mn in mnames:
        ...
        for on in mnames:
            ...
            for target in targets:
                ...
                if auc:
                    iou_data['AUC'].append(auc[(mn,target)])
    ...
    if len(targets) <= 20:
        ...
        ax.set_ylabel('Intersection Over Union%s' %ylabel)
        ax.set_xlabel('')
        ax.set(ylim=(-0.025, 1.025))
        fig.savefig('%siou%s_differentmarkers_boxplot.pdf' %(prefix,out), bbox_inches='tight')
    if auc:
        fig,ax = plt.subplots(figsize=(12.8,9.6))
        threshold = 0.6
        relevant = iou_data.loc[iou_data.AUC < threshold]
        ...
        fig.savefig('%siou%s_boxplot.pdf' %(prefix,out), bbox_inches='tight')
    if len(targets) > 20 and not auc:
        fig,ax = plt.subplots(figsize=(12.8,9.6))
        sns.stripplot(x='Method1', y='IOU', hue='Method2',
                data=iou_data, split=True, size=6, jitter=True, 
...
if not args.affinity:
    ...
    # intersection/union for top 50 mols per target
    topn = grouped.apply(lambda x: x.nlargest(50, 'Prediction'))
    topn.reset_index(inplace=True, drop=True)
    plot_iou(topn, palette, args.prefix, '_top50', '\nTop 50 Compounds', aucs)

    # intersection/union for top 10% of actives per target
    grouped = df.loc[df['True']==1].groupby(['Method', 'Target'])
    topn = grouped.apply(lambda x: x.nlargest(math.ceil(len(x) * .1), 'Prediction'))
    topn.reset_index(inplace=True, drop=True)
    plot_iou(topn, palette, args.prefix, '_topactives', '\nTop 10\% of Actives', aucs)
    
    # intersection/union for bottom 10% of actives per target
    grouped = df.loc[df['True']==1].groupby(['Method', 'Target'])
    topn = grouped.apply(lambda x: x.nsmallest(math.ceil(len(x) * .1), 'Prediction'))
    topn.reset_index(inplace=True, drop=True)
    plot_iou(topn, palette, args.prefix, '_worstactives', '\nBottom 10\% of Actives', aucs)

    # intersection/union for top 10% of inactives per target
    grouped = df.loc[df['True']==0].groupby(['Method', 'Target'])
    topn = grouped.apply(lambda x: x.nlargest(math.ceil(len(x) * .1), 'Prediction'))
    topn.reset_index(inplace=True, drop=True)
    plot_iou(topn, palette, args.prefix, '_topinactives', '\nTop 10\% of Inactives', aucs)

    # intersection/union for bottom 10% of inactives per target
    grouped = df.loc[df['True']==0].groupby(['Method', 'Target'])
    topn = grouped.apply(lambda x: x.nsmallest(math.ceil(len(x) * .1), 'Prediction'))
    topn.reset_index(inplace=True, drop=True)
    plot_iou(topn, palette, args.prefix, '_worstinactives', '\nBottom 10\% of Inactives', aucs)
